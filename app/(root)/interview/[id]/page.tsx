...
export default function InterviewDetails({ params, searchParams }: PageProps): React.ReactElement {
  ...
  const { user, loading: authLoading } = useAuth(); // Renamed for clarity
  const [interview, setInterview] = useState<Interview | null>(null);
  const [feedback, setFeedback] = useState<any>(null);
  const [isLoadingData, setIsLoadingData] = useState(true); // Specific for page data
  const [fetchError, setFetchError] = useState<string | null>(null); // Error state

  useEffect(() => {
    const fetchData = async () => {
      if (!user || !id) { // Ensure id from params is also available
        setIsLoadingData(false);
        if (!user && !authLoading) router.push("/sign-in"); // Redirect if auth is resolved and no user
        return;
      }
      ...
      try {
        ...
        const interviewData = await request("GET", `/interviews/${id}`);
        if (!interviewData) {
          setFetchError("Interview not found or could not be loaded.");
          setInterview(null); // Ensure interview state is null if not found
          setIsLoadingData(false);
          // router.push("/"); // Optionally redirect, or let the render logic handle 'interview not found'
          return;
        }
        setInterview(interviewData);

        const feedbackData = await request("GET", `/feedback/interview/${id}`);
    ...
    if (!authLoading && id) { // Start fetching when auth is done and id is available
      fetchData();
    }
  ...
  // Memoize derived data (already present, just ensuring it's after loading/error checks)
  ...
  // Memoize questions prop for Agent to prevent unnecessary re-renders of Agent
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <InterviewHeader
          role={interview.role}
          imageSrc={imageSrc}
          questionCount={agentQuestions?.length || 0} // Use agentQuestions
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              <Agent
                userName={user?.username || ""} // Consider memoizing user-related props if user object is complex/changes often
                userId={user?.id}
                userAvatar={user?.photoURL}
                interviewId={id} // id is stable from params
                type="interview" // type is a static string
                questions={agentQuestions} // Pass memoized questions
              />
            </div>
          </div>
