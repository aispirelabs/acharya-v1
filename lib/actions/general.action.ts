...
export async function getLatestInterviews(
  ...
  // Use a default limit of 10 if not provided or invalid
  const defaultLimit = 10;
  const { userId, limit = defaultLimit } = params;
  
  // Ensure limit is a valid number
  const validLimit = typeof limit === 'number' && !isNaN(limit) ? limit : defaultLimit;
  ...
  return interviews.docs.map((doc: QueryDocumentSnapshot<DocumentData>) => ({
    id: doc.id,
    ...doc.data(),
  })) as Interview[];
}
...
export async function getFeedbacksByInterviewId(
  ...
  const { limit, offset, countOnly } = params;
